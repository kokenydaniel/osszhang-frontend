...
import { useAppStore, Meter, MeterReading } from '@/lib/store';
...
function getChartData(meter: Meter, selectedYear: number, getPreviousYearValue: any) {
  const result = [];
  for(let m = 1; m <= 12; m++) {
    const cyData = meter.readings.find((r: any) => r.month === m && r.year === selectedYear);
...
function MeterCard({ meter, selectedYear, getPreviousYearValue, deleteMeterReading, onAiClick, onEditReading, onAddReading, onDeleteMeter, onDeleteReading }: { meter: Meter, selectedYear: number, getPreviousYearValue: any, deleteMeterReading: any, onAiClick: (id: number) => void, onEditReading: (m: Meter, r: MeterReading) => void, onAddReading: (m: Meter) => void, onDeleteMeter: (id: number) => void, onDeleteReading: (mId: number, rId: number) => void }) {
  const [showFullHistory, setShowFullHistory] = useState(false);
  const chartData = getChartData(meter, selectedYear, getPreviousYearValue);
  
  const yearReadings = meter.readings.filter((r: any) => r.year === selectedYear).sort((a:any, b:any) => new Date(b.date).getTime() - new Date(a.date).getTime());
  const otherReadings = meter.readings.filter((r: any) => r.year !== selectedYear).sort((a:any, b:any) => new Date(b.date).getTime() - new Date(a.date).getTime());
...
export default function MetersClient() {
  const { meters, addMeter, deleteMeter, addMeterReading, updateMeterReading, deleteMeterReading, selectedMonth, selectedYear, aiUtilityAnomalies, fetchAiUtilityAnomalies } = useAppStore();
...
3) Válaszod CSAK egyetlen egész szám legyen (szöveg és mértékegység nélkül).`;
        ...
        const res = await aiApi.query(prompt, false);
      ...
      {!!aiUtilityAnomalies?.anomalies?.length && (
        <div className="p-4 bg-amber-500/5 border border-amber-500/20 rounded-2xl flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1 text-sm text-slate-300">
            {aiUtilityAnomalies.anomalies.map((a: any) => (
