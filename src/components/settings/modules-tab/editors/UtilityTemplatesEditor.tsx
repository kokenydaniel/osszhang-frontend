...
import type { UtilityTemplate } from '@/config/utility-templates';
      ...
      {value.length === 0 ? (
        ...
        <div className="space-y-2">
          {value.map((row, index) => (
            ...
            >
            <div className="grid grid-cols-1 sm:grid-cols-[1fr_100px_72px_140px_auto] gap-2 items-end"
            >
                ...
                >
                  {SPLIT_RULES.map((rule) => (
                    <option key={rule} value={rule}>
                      {utilitiesCalculations.splitRuleLabel(rule, true)}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <div className="space-y-1">
                <span className="text-[0.65rem] font-medium uppercase tracking-wider text-muted-foreground">Szolgáltató</span>
                <Input
                  value={row.provider ?? ''}
              ...
              <div className="space-y-1">
                <span className="text-[0.65rem] font-medium uppercase tracking-wider text-muted-foreground">Fizetési mód</span>
                <Input
                  value={row.payment_method ?? ''}
                  onChange={(e) => updateRow(index, { payment_method: e.target.value })}
                  placeholder="pl. csoportos beszedés"
                />
              </div>
              <div className="space-y-1">
                <span className="text-[0.65rem] font-medium uppercase tracking-wider text-muted-foreground">Költségvetés kategória</span>
                <Input
                  value={row.budget_category ?? ''}
                  onChange={(e) => updateRow(index, { budget_category: e.target.value })}
                  placeholder="pl. Rezsi"
                />
              </div>
            </div>
            </div>
