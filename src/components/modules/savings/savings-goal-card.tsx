...
export function SavingsGoalCard({
  ...
  const walletIsShared = goal.wallet?.isShared ?? true;
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 pt-1 border-t border-border/60">
        <MiniSwitch
          ...
          onChange={(checked) => updateSavingsAccount(goal.id, { count_in_savings: checked })}
          label="Vagyonba"
          title="Beleszámít a fő vagyon összegébe a Széf nézetben"
          disabled={isReader}
        />
        <Button type="button" variant="ghost" size="xs" onClick={() => openLedgerModal(goal.id)}>
