...
import { dayjs, d, formatTodayLong, toDateString } from '@/lib/dates';
...
import type {
  ...
  DashboardUnpaidItem,
} from '@/components/modules/dashboard/lib/dashboardTypes';
...
function estimateInvestmentPayout(inv: Investment): Omit<DashboardInvestmentPayout, 'invName' | 'owner'> | null {
  ...
  const now = dayjs();
  const maturityAmount = SavingsService.computeMaturityAmount(inv);
    ...
    spending.push(
      transactions
        ...
        bills
          ...
          .reduce((s, b) => s + UtilitiesService.ourUtilityPortion(b, onHouseholdSide, utilitySplitEnabled), 0),
    );
    income.push(
      transactions
...
export function computeDashboardSnapshot(input: DashboardSnapshotInput) {
  ...
  const utilityBills = input.bills.filter((b) => !UtilitiesService.isLegacySettlementBill(b));
  ...
  const { netBalance: rawRezsiBalance } = UtilitiesService.computeNetBalance(
    monthBills,
    input.user?.id,
    input.partnerId,
  ...
  const unpaidBills = input.bills.filter((b) => !b.paidDate && b.dueDate.startsWith(prefix));
  ...
  const unpaidItemsList: DashboardUnpaidItem[] = [
    ...
    ...(input.canUse('utilities')
      ? monthBills.filter((b) => !b.paidDate).map((b) => ({
          ...
          amount: UtilitiesService.ourUtilityPortion(b, input.onHouseholdSide, input.utilitySplitEnabled),
          dueDate: b.dueDate,
          category: 'Rezsi',
        }))
      : []),
