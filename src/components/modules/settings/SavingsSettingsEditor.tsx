  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-1.5 min-h-[2rem]">
            {items.map((item) => (
              <span key={item} className="inline-flex items-center gap-1 rounded-md border border-border bg-muted/30 px-2 py-1 text-xs font-medium">
                {item}
                <button type="button" onClick={() => onChange(items.filter((x) => x !== item))} className="text-muted-foreground hover:text-destructive">
                  <X size={12} />
                </button>
              </span>
            ))}
