...
import { CustomTooltip } from '@/components/modules/meters/meter-chart-utils';
...
export function MeterPanel({
  ...
  onExpandFullHistory: () => void;
  onCalcValueChange: (value: string) => void;
  ...
  const chartData = MetersService.buildChartData(meter, selectedYear, getPreviousYearValue);
  const meta = MetersService.getMeterMeta(meter.name);
  const { Icon, iconBg, surface, bar, accent } = meta;
  const { yearTotal, prevYearTotal, trend } = MetersService.buildYearTotals(meter, selectedYear);
  const { latestReading, latestOfficialReading, consumptionSinceOfficial } =
    MetersService.buildConsumptionSinceOfficial(meter);
  ...
  const diff = latestReading && !isNaN(currentVal) ? MetersService.computeQuickReadingDiff(latestReading.value, currentVal) : 0;
              ...
              <div className="flex items-center gap-2">
                <div className="relative">
                  <Input
                    ...
                    value={calcValue}
                    onChange={(e) => onCalcValueChange(e.target.value)}
