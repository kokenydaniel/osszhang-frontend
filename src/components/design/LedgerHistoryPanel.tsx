...
interface LedgerHistoryPanelProps {
  ...
  rowFooter?: (item: LedgerHistoryItem) => React.ReactNode;
}
            ...
            .map((item) => (
              <div
                ...
                className={classNames(
                  'bg-muted/40 border border-border rounded-md px-3 py-2',
              ...
              >
                ...
                {rowFooter ? <div className="mt-2 border-t border-border/60 pt-2">{rowFooter(item)}</div> : null}
              </div>
            ))
        )}
      </div>
