...
export function mapHouseholdFromApi(h: NonNullable<RawApiUser['household']>): NonNullable<UserProfile['household']> {
  return {
    ...
    business_enabled: h.business_enabled,
    businessEnabled: h.business_enabled ?? h.businessEnabled ?? true,
    business_name: h.business_name,
    businessName: h.business_name ?? h.businessName ?? 'Little Loom',
    ...
    utilitySplitEnabled: h.utility_split_enabled ?? h.utilitySplitEnabled ?? true,
