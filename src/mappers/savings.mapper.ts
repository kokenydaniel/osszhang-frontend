...
// These represent the raw JSON shapes returned by the backend before mapping.

export type RawSavingsAccount = {
  id: number;
  type?: SavingsType;
...
// ─── Mappers (pure functions: DTO → Frontend Model) ───────────────────────────

/**
 * Maps a single raw API savings account to the strongly-typed frontend model.
 * This is the single source of truth for the API↔frontend field name mapping.
 */
export function mapSavingsAccountFromApi(raw: RawSavingsAccount): SavingsAccount {
  ...
  if (type === 'goal') {
    // For goals, currentAmount is always derived from the ledger sum (absolute values)
    currentAmount =
      ledger.length === 0
        ? 0
...
/**
 * Maps an array of raw API savings accounts.
 */
...
/**
 * Converts a partial frontend model to the API-safe payload shape (snake_case).
 */
